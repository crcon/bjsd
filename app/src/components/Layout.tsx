...
export function Layout({ children, activeTab, onTabChange }: LayoutProps) {
  ...
  const footerContent = (
    <>
      <p className="text-xs font-semibold text-slate-700">© 易储数能开发平台</p>
      <p className="mt-1 text-xs text-slate-500">联系人：Jack.c</p>
      <p className="text-xs text-slate-500">电话：13980081510</p>
    </>
  );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 flex">
      ...
      <main className="flex-1 min-w-0 overflow-auto flex flex-col">
        ...
        <footer className="border-t border-slate-200 bg-white/80 px-6 py-4 backdrop-blur">
          {footerContent}
        </footer>
      </main>
    </div>
